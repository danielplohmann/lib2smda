...
def libfile2smda(converter, input_filepath, output_folder, output_filename):
    release_hashes = get_release_hashes(input_filepath)
    extracted_obj_files = {}
    if input_filepath.lower().endswith(".o") or input_filepath.lower().endswith(".obj"):
    ...
    if merged_report:
        ...
        merged_report.family = "nim"
        merged_report.version = "1.6.14"
...
if __name__ == "__main__":
    ...
    for filename in sorted(os.listdir(input_abspath)):
        filepath = input_abspath + os.sep + filename
        print(f"processing: ", filepath)
        release_hashes = get_release_hashes(filepath)
        output_path = this_path + os.sep + "output"
        converter.ensureDirExists(output_path)
        output_filename = f"{filename}.smda"
        libfile2smda(converter, filepath, output_path, output_filename)
